...
`client.py` and `server.py` implement the sender and the receiver logic of Go-Back-N ARQ protocol respectively 
...
Next, run the `client.py` as following. The client runs in localhost and is bound to a customary port. The server port is mentioned as `7735` here, same as the port where the server is running. `testfile.txt` is the file that the client wishes to send. The first value following the input file denotes the sender window size = 4. MSS follows the sender window size = 4.
```
# python3 client.py localhost 7735 testfile.txt 4 4
```
