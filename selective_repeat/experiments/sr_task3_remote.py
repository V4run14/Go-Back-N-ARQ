"""
Selective Repeat Task 3 runner against a remote server: vary loss probability p.

Assumes sr_server.py (or sr_server_task3.py if cycling) is running on the remote host with:
    python3 sr_server.py <port> <output_file> <p> <window_size>
"""
import argparse
import csv
import sys
...
def main():
    parser = argparse.ArgumentParser(
        description="Run SR Task 3 (loss sweep) against a remote server.")
    ...
    parser.add_argument("--output", type=Path, default=Path("sr_task3_remote.csv"),
    ...
    rows = []
    print(f"Running SR Task 3 remote sweep to {args.host}:{args.port}")
    print(f"File={args.file} Window={args.window} MSS={args.mss} Trials={args.trials}")
    for loss in LOSS_VALUES:
        print(f"NOTE: Ensure server p={loss} (or use sr_server_task3.py to auto-cycle).")
